...
//                                     .tx_queue_len = 10, //发送队列长度，0-禁用发送队列
...
void consoleTwaiStatus(int type) {
  ...
  if(type == 3) {
    ESP_LOGE("TWAI 状态", "state: %u  发送队列(msgs_to_tx): %lu  -接收队列(msgs_to_tx), %lu -发送错误计数(tx_error_counter): %lu  -接收错误计数(rx_error_counter): %lu  ", status_info.state, status_info.msgs_to_tx, status_info.msgs_to_rx, status_info.tx_error_counter, status_info.rx_error_counter);
    ESP_LOGE("TWAI 状态", "-发送失败计数(tx_failed_count): %lu  -接收失败计数(rx_missed_count): %lu -接收队列的溢出丢失数(rx_overrun_count): %lu -仲裁失败计数(arb_lost_count): %lu -总线错误计数(bus_error_count): %lu  \n", status_info.tx_failed_count, status_info.rx_missed_count,status_info.rx_overrun_count, status_info.arb_lost_count, status_info.bus_error_count);
  }
  else if(type == 2) {
    ESP_LOGW("TWAI 状态", "state: %u  发送队列(msgs_to_tx): %lu  -接收队列(msgs_to_tx), %lu -发送错误计数(tx_error_counter): %lu  -接收错误计数(rx_error_counter): %lu  ", status_info.state, status_info.msgs_to_tx, status_info.msgs_to_rx, status_info.tx_error_counter, status_info.rx_error_counter);
    ESP_LOGW("TWAI 状态", "-发送失败计数(tx_failed_count): %lu  -接收失败计数(rx_missed_count): %lu -接收队列的溢出丢失数(rx_overrun_count): %lu -仲裁失败计数(arb_lost_count): %lu -总线错误计数(bus_error_count): %lu  \n", status_info.tx_failed_count, status_info.rx_missed_count,status_info.rx_overrun_count, status_info.arb_lost_count, status_info.bus_error_count);
  }else {
    ESP_LOGI("TWAI 状态", "state: %u  发送队列(msgs_to_tx): %lu  -接收队列(msgs_to_tx), %lu -发送错误计数(tx_error_counter): %lu  -接收错误计数(rx_error_counter): %lu  ", status_info.state, status_info.msgs_to_tx, status_info.msgs_to_rx, status_info.tx_error_counter, status_info.rx_error_counter);
    ESP_LOGI("TWAI 状态", "-发送失败计数(tx_failed_count): %lu  -接收失败计数(rx_missed_count): %lu -接收队列的溢出丢失数(rx_overrun_count): %lu -仲裁失败计数(arb_lost_count): %lu -总线错误计数(bus_error_count): %lu  \n", status_info.tx_failed_count, status_info.rx_missed_count,status_info.rx_overrun_count, status_info.arb_lost_count, status_info.bus_error_count);
  }
}
...
void emitMsg(long unsigned int id,  uint8_t dataArr[]) {
  ...
  printf("emitMsg 帧 frameID= %lu 数组：", id);
  for(int i = 0; i < 8; i++) {
    msg.data[i] = dataArr[i];
    printf("%hhu ", dataArr[i]);
  }
